...
func testCertificationsExists(t *testing.T) {
	...
	e, err := CertificationExists(ctx, tx, o.ID)
	if err != nil {
...
func testCertificationsFind(t *testing.T) {
	...
	certificationFound, err := FindCertification(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testCertificationToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Certification
	var foreign User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, certificationDBTypes, false, certificationColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Certification struct: %s", err)
	}
	...
	slice := CertificationSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Certification)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testCertificationToOneSetOpUserUsingUser(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, certificationDBTypes, false, strmangle.SetComplement(certificationPrimaryKeyColumns, certificationColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
var (
	certificationDBTypes = map[string]string{`ID`: `varchar`, `Name`: `varchar`, `WhenDate`: `datetime`, `Comment`: `text`, `SortIndex`: `int`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`, `UserID`: `varchar`}
	_                    = bytes.MinRead
)
