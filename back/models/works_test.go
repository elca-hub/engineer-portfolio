...
func testWorkToManyWorkHavingTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, workDBTypes, true, workColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Work struct: %s", err)
	}
	...
	slice := WorkSlice{&a}
	if err = a.L.LoadWorkHavingTags(ctx, tx, false, (*[]*Work)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testWorkToManyWorkImages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, workImageDBTypes, false, workImageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, workImageDBTypes, false, workImageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.WorkImages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := WorkSlice{&a}
	if err = a.L.LoadWorkImages(ctx, tx, false, (*[]*Work)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.WorkImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.WorkImages = nil
	if err = a.L.LoadWorkImages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.WorkImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testWorkToManyWorkUrls(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, workURLDBTypes, false, workURLColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, workURLDBTypes, false, workURLColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.WorkUrls().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := WorkSlice{&a}
	if err = a.L.LoadWorkUrls(ctx, tx, false, (*[]*Work)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.WorkUrls); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.WorkUrls = nil
	if err = a.L.LoadWorkUrls(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.WorkUrls); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testWorkToManyAddOpWorkHavingTags(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, workDBTypes, false, strmangle.SetComplement(workPrimaryKeyColumns, workColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
func testWorkToManyAddOpWorkImages(t *testing.T) {
	var err error
	...
	var a Work
	var b, c, d, e WorkImage

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, workDBTypes, false, strmangle.SetComplement(workPrimaryKeyColumns, workColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*WorkImage{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, workImageDBTypes, false, strmangle.SetComplement(workImagePrimaryKeyColumns, workImageColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*WorkImage{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddWorkImages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.WorkImages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.WorkImages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.WorkImages().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testWorkToManyAddOpWorkUrls(t *testing.T) {
	var err error
	...
	foreigners := []*WorkURL{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, workURLDBTypes, false, strmangle.SetComplement(workURLPrimaryKeyColumns, workURLColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*WorkURL{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddWorkUrls(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.WorkUrls[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.WorkUrls[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.WorkUrls().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testWorkToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, workDBTypes, false, workColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Work struct: %s", err)
	}
	...
	slice := WorkSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Work)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testWorkToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	workDBTypes = map[string]string{`ID`: `varchar`, `Title`: `varchar`, `SortIndex`: `int`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`, `UserID`: `varchar`, `Content`: `text`, `GithubRepositoryURL`: `varchar`, `IsDraft`: `tinyint`}
	_           = bytes.MinRead
)
...
func testWorksUpsert(t *testing.T) {
	...
	if len(mySQLWorkUniqueColumns) == 0 {
