...
type CreateCertificationAction struct {
	uc certification.CreateCertificationUseCase
	v  validator.Validator
	l  logger.Logger
}

func NewCreateCertificationAction(
	uc certification.CreateCertificationUseCase,
	v validator.Validator, l logger.Logger,
) *CreateCertificationAction {
	return &CreateCertificationAction{
		uc: uc,
		v:  v,
		l:  l,
	}
...
func (a *CreateCertificationAction) Execute(w http.ResponseWriter, r *http.Request, c *gin.Context) {
	var input certification.CreateCertificationInput
	const logKey = "create_certification"
	...
	if err != nil {
		logging.NewError(a.l, err, logKey, http.StatusInternalServerError).Log("error when create certification")
		response.NewError(err, http.StatusInternalServerError).Send(w)
		return
	}

	response.NewSuccess(output, http.StatusOK).Send(w)
	logging.NewInfo(a.l, logKey, http.StatusOK).Log("success create certification")
}