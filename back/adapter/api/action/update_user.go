...
func (a *UpdateUserAction) Execute(w http.ResponseWriter, r *http.Request, c *gin.Context) {
	...
	if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
		logging.NewError(a.l, err, logKey, http.StatusBadRequest).Log("error when decode update user request")
		response.NewError(err, http.StatusBadRequest).Send(w)
		return
	}

	if err := a.v.Validate(input); err != nil {
		logging.NewError(a.l, err, logKey, http.StatusBadRequest).Log("error when validate update user request")
		response.NewError(err, http.StatusBadRequest).Send(w)
		return
	}

	input.User.UserId = currentUser.ID()
	input.User.Email = currentUser.Email().Email()
