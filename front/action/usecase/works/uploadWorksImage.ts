...
export default async function uploadWorksImage(token: string, workId: string, image: File): Promise<
...
> {
  ...
  const formData = new FormData()
	formData.append('work_image', image)

	return await NewDPResponse<{ image_url: string }>(
		await fetch(`${apiPrefix}/auth/user/works/${workId}/image`, {
			method: 'POST',
			body: formData,
			headers: {
